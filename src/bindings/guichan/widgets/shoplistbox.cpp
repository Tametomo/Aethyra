...
ShopListBox::ShopListBox(gcn::ListModel *listModel, ShopListModel *shopListModel):
    ListBox(listModel),
    mPlayerMoney(0),
...
void ShopListBox::draw(gcn::Graphics *gcnGraphics)
{
         ...
         ++i, y += mRowHeight)
    {
        ...
        if (mShopListModel &&
                mPlayerMoney < mShopListModel->at(i)->getPrice() && mPriceCheck)
            if (i != mSelected)
        ...
        if (mShopListModel)
        {
            Image *icon = mShopListModel->at(i)->getImage();
            if (icon)
            {
